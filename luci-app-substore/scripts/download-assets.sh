...
backend)
	...
	if [ -n "$BACKEND_TAG_LIVE" ]; then
		...
		echo "官方 API 查询成功，后端版本号: $BACKEND_TAG_LIVE"
	else
		echo "unknown" > "$LIBEXEC_DIR/backend.version"
		echo "官方 API 查询失败，标记为 unknown" >&2
	fi
	;;
...
frontend)
	...
	if [ -n "$FRONTEND_TAG_LIVE" ]; then
		...
		echo "官方 API 查询成功，前端版本号: $FRONTEND_TAG_LIVE"
	else
